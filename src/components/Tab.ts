...
export default (editor: Editor, config: TabConfig) => {
  ...
  const getPrevCntId = (comp: Component) =>
    comp.getAttributes()[prevSelectorTab];
  ...
  Components.addType(config.typeTab, {
    model: {
      ...
      clone(
        opt: { _inner?: boolean; symbol?: boolean; symbolInv?: boolean } = {}
      ) {
        ...
        !fromParent && cloned.addAttributes({ [config.selectorTab]: "" });
        return cloned;
      },
    },
  });
